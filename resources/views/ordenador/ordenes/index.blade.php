...
@section('title', 'Órdenes Autorizadas - Ordenador')
...
<div class="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-8">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-gray-600 text-lg">Cuentas de cobro que has autorizado y están en proceso de pago</p>
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
            <!-- Estadística: Total Autorizadas -->
            <div class="bg-white/70 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-white/20">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-600">Total Autorizadas</p>
                        <p class="text-3xl font-bold text-green-600">{{ $ordenes->total() }}</p>
                    </div>
            ...
            <div class="bg-white/70 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-white/20">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-blue-600">${{ number_format($valorTotal, 0, ',', '.') }}</p>
                    </div>
            ...
            <!-- Filtro por Usuario -->
            <div class="bg-white/70 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-white/20">
                <form method="GET" action="{{ route('ordenador.ordenes.index') }}">
        ...
        <div class="bg-white/70 backdrop-blur-sm rounded-xl shadow-lg border border-white/20 overflow-hidden">
                    ...
                    <table class="w-full">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-200">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900 max-w-xs truncate" title="{{ $orden->proyecto_servicio }}">
                                        {{ $orden->proyecto_servicio }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-green-600">
                                    ${{ number_format($orden->valor, 0, ',', '.') }}
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                    <a href="{{ route('ordenador.ordenes.show', $orden->id) }}" 
            ...
            @else
                ...
                <div class="text-center py-12">
                    <div class="mb-4">
                        <i class="fas fa-file-check text-6xl text-gray-300"></i>
                    </div>
                    <h3 class="text-xl font-semibold text-gray-500 mb-2">No hay órdenes autorizadas</h3>
                    <p class="text-gray-400 mb-6">
                        @if(request('usuario') || request('fecha'))
                            No se encontraron órdenes con los filtros aplicados.
                        @else
                            Aún no has autorizado ninguna cuenta de cobro.
                        @endif
                    </p>
                    @if(request('usuario') || request('fecha'))
                    <a href="{{ route('ordenador.ordenes.index') }}" 
        ...
        <div class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white/70 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-white/20 text-center">
                <div class="text-3xl font-bold text-green-600 mb-2">{{ $ordenesEsteMes }}</div>
                <p class="text-gray-600">Autorizadas este mes</p>
            </div>
            <div class="bg-white/70 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-white/20 text-center">
                <div class="text-3xl font-bold text-blue-600 mb-2">${{ number_format($valorPromedio, 0, ',', '.') }}</div>
                <p class="text-gray-600">Valor promedio</p>
            </div>
            <div class="bg-white/70 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-white/20 text-center">
                <div class="text-3xl font-bold text-purple-600 mb-2">{{ $diasPromedio }}</div>
                <p class="text-gray-600">Días promedio de autorización</p>
            </div>
        </div>
        @endif
    </div>
